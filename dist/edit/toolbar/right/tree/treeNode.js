...
let TreeNode = React.createClass({
    ...
    render: function () {
        ...
        if (this.state.childs.length > 0) {
            if (expand) {
                angle = React.createElement('i', { onClick: this.hideChildren, className: 'fa fa-angle-down' });
            } else {
                angle = React.createElement('i', { onClick: this.showChildren, className: 'fa fa-angle-right' });
            }
        }
        ...
        let childNodes = this.state.childs.map((item, index) => {
            ...
            return React.createElement(
                'div',
                { className: 'node', key: item.uniqueKey },
                React.createElement(TreeNode, childProps)
            );
        });
        ...
        return React.createElement(
            ...
            React.createElement(
                ...
                React.createElement(
                    'div',
                    { className: classNames('children', 'none-select', { 'show': expand, 'hide': !expand }) },
                    childNodes
                )
            )
        );
